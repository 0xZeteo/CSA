...
class CSM_Domain3_Page(tk.Frame):
    def __init__(self, master):
        ...
        self.create_category_card(
            col1_frame,
            "Preventive Controls",
            csm.submit_pressed(CSM_Domain3_PreventiveControls_Page.values)[3],
            sum([len(DATA.CSM_Domain3_PreventiveControls[i]) for i in DATA.CSM_Domain3_PreventiveControls]),
            lambda: master.switch_frame(CSM_Domain3_PreventiveControls_Page)
        )
        
        self.create_category_card(
            col2_frame,
            "Detective Controls",
            csm.submit_pressed(CSM_Domain3_DetectiveControls_Page.values)[3],
            sum([len(DATA.CSM_Domain3_DetectiveControls[i]) for i in DATA.CSM_Domain3_DetectiveControls]),
            lambda: master.switch_frame(CSM_Domain3_DetectiveControls_Page)
        )
        
        self.create_category_card(
            col3_frame,
            "Corrective Controls",
            csm.submit_pressed(CSM_Domain3_CorrectiveControls_Page.values)[3],
            sum([len(DATA.CSM_Domain3_CorrectiveControls[i]) for i in DATA.CSM_Domain3_CorrectiveControls]),
            lambda: master.switch_frame(CSM_Domain3_CorrectiveControls_Page)
        )
...
class CSM_Domain3_PreventiveControls_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain3_PreventiveControls[i]) for i in DATA.CSM_Domain3_PreventiveControls])
...
class CSM_Domain3_DetectiveControls_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain3_DetectiveControls[i]) for i in DATA.CSM_Domain3_DetectiveControls])
            for _ in range(total_questions):
                cls.values.append(tk.IntVar())
...
class CSM_Domain3_CorrectiveControls_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain3_CorrectiveControls[i]) for i in DATA.CSM_Domain3_CorrectiveControls])
            for _ in range(total_questions):
                cls.values.append(tk.IntVar())
