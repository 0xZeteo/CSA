...
class CSM_Domain4_Page(tk.Frame):
    def __init__(self, master):
        ...
        title_label = tk.Label(
            header_frame, 
            text="External Dependency Management",
            font=("Arial", 24, "bold"),
            bg=COLORS["bg"],
            fg=COLORS["primary"]
        )
        ...
        self.create_category_card(
            col1_frame,
            "Connections",
            csm.submit_pressed(CSM_Domain4_Connections_Page.values)[3],
            sum([len(DATA.CSM_Domain4_Connections[i]) for i in DATA.CSM_Domain4_Connections]),
            lambda: master.switch_frame(CSM_Domain4_Connections_Page)
        )
        
        self.create_category_card(
            col2_frame,
            "Relationship Management",
            csm.submit_pressed(CSM_Domain4_RelationshipManagement_Page.values)[3],
            sum([len(DATA.CSM_Domain4_RelationshipManagement[i]) for i in DATA.CSM_Domain4_RelationshipManagement]),
            lambda: master.switch_frame(CSM_Domain4_RelationshipManagement_Page)
        )
...
class CSM_Domain4_Connections_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain4_Connections[i]) for i in DATA.CSM_Domain4_Connections])
...
class CSM_Domain4_RelationshipManagement_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain4_RelationshipManagement[i]) for i in DATA.CSM_Domain4_RelationshipManagement])
            for _ in range(total_questions):
                cls.values.append(tk.IntVar())
