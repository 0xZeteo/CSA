...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 5 page """
class CSM_Domain5_Page(tk.Frame):
    def __init__(self, master):
        ...
        title_label = tk.Label(
            header_frame, 
            text="Cyber Incident Management and Resilience",
            font=("Arial", 24, "bold"),
            bg=COLORS["bg"],
            fg=COLORS["primary"]
        )
        ...
        # Center column in second row
        col3_frame = tk.Frame(row2_frame, bg="white")
        col3_frame.pack(fill="both", expand=True, padx=100)  # Add padding to make it appear centered
        
        # Create category cards in grid layout
        self.create_category_card(
            col1_frame,
            "Incident Planning and Strategy",
            csm.submit_pressed(CSM_Domain5_IncidentPlanningAndStrategy_Page.values)[3],
            sum([len(DATA.CSM_Domain5_IncidentPlanningAndStrategy[i]) for i in DATA.CSM_Domain5_IncidentPlanningAndStrategy]),
            lambda: master.switch_frame(CSM_Domain5_IncidentPlanningAndStrategy_Page)
        )
        
        self.create_category_card(
            col2_frame,
            "Detection Response and Mitigation",
            csm.submit_pressed(CSM_Domain5_DetectionResponseAndMitigation_Page.values)[3],
            sum([len(DATA.CSM_Domain5_DetectionResponseAndMitigation[i]) for i in DATA.CSM_Domain5_DetectionResponseAndMitigation]),
            lambda: master.switch_frame(CSM_Domain5_DetectionResponseAndMitigation_Page)
        )
        
        self.create_category_card(
            col3_frame,
            "Escalation and Reporting",
            csm.submit_pressed(CSM_Domain5_EscalationAndReporting_Page.values)[3],
            sum([len(DATA.CSM_Domain5_EscalationAndReporting[i]) for i in DATA.CSM_Domain5_EscalationAndReporting]),
            lambda: master.switch_frame(CSM_Domain5_EscalationAndReporting_Page)
        )
...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 5 - Incident Planning and Strategy page """
class CSM_Domain5_IncidentPlanningAndStrategy_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain5_IncidentPlanningAndStrategy[i]) for i in DATA.CSM_Domain5_IncidentPlanningAndStrategy])
    ...
    def __init__(self, master):
        ...
        title_label = tk.Label(
            title_frame,
            text="Incident Planning and Strategy",
            font=("Arial", 16, "bold"),
            bg="white",
            fg=COLORS["primary"]
        )
...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 5 - Detection Response and Mitigation page """
class CSM_Domain5_DetectionResponseAndMitigation_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain5_DetectionResponseAndMitigation[i]) for i in DATA.CSM_Domain5_DetectionResponseAndMitigation])
            for _ in range(total_questions):
                cls.values.append(tk.IntVar())
    ...
    def __init__(self, master):
        ...
        title_label = tk.Label(
            title_frame,
            text="Detection Response and Mitigation",
            font=("Arial", 16, "bold"),
            bg="white",
            fg=COLORS["primary"]
        )
...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 5 - Escalation and Reporting page """
class CSM_Domain5_EscalationAndReporting_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain5_EscalationAndReporting[i]) for i in DATA.CSM_Domain5_EscalationAndReporting])
            for _ in range(total_questions):
                cls.values.append(tk.IntVar())
