...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 2 page """
class CSM_Domain2_Page(tk.Frame):
    def __init__(self, master):
        ...
        title_label = tk.Label(
            header_frame, 
            text="Threat Intelligence and Collaboration",
            font=("Arial", 24, "bold"),
            bg=COLORS["bg"],
            fg=COLORS["primary"]
        )
        ...
        self.create_category_card(
            col1_frame,
            "Threat Intelligence",
            csm.submit_pressed(CSM_Domain2_ThreatIntelligence_Page.values)[3],
            sum([len(DATA.CSM_Domain2_ThreatIntelligence[i]) for i in DATA.CSM_Domain2_ThreatIntelligence]),
            lambda: master.switch_frame(CSM_Domain2_ThreatIntelligence_Page)
        )
        
        self.create_category_card(
            col2_frame,
            "Monitoring and Analyzing",
            csm.submit_pressed(CSM_Domain2_MonitoringAndAnalyzing_Page.values)[3],
            sum([len(DATA.CSM_Domain2_MonitoringAndAnalyzing[i]) for i in DATA.CSM_Domain2_MonitoringAndAnalyzing]),
            lambda: master.switch_frame(CSM_Domain2_MonitoringAndAnalyzing_Page)
        )
        
        self.create_category_card(
            col3_frame,
            "Information Sharing",
            csm.submit_pressed(CSM_Domain2_InformationSharing_Page.values)[3],
            sum([len(DATA.CSM_Domain2_InformationSharing[i]) for i in DATA.CSM_Domain2_InformationSharing]),
            lambda: master.switch_frame(CSM_Domain2_InformationSharing_Page)
        )
...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 2 - Threat Intelligence page """
class CSM_Domain2_ThreatIntelligence_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain2_ThreatIntelligence[i]) for i in DATA.CSM_Domain2_ThreatIntelligence])
...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 2 - Monitoring and Analyzing page """
class CSM_Domain2_MonitoringAndAnalyzing_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain2_MonitoringAndAnalyzing[i]) for i in DATA.CSM_Domain2_MonitoringAndAnalyzing])
            for _ in range(total_questions):
                cls.values.append(tk.IntVar())
...
""" This class is responsible for the layout of the Cybersecurity Maturity's Domain 2 - Information Sharing page """
class CSM_Domain2_InformationSharing_Page(tk.Frame):
    ...
    def initialize_values(cls):
        if not cls.values:
            total_questions = sum([len(DATA.CSM_Domain2_InformationSharing[i]) for i in DATA.CSM_Domain2_InformationSharing])
            for _ in range(total_questions):
                cls.values.append(tk.IntVar())
